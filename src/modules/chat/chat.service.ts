...
export class ChatService {
  ...
  async createNewChat(messageDto: MessageDto) {
    const room = await this.findRoomById(messageDto.roomId);
    const user = await this.userService.findUserById(messageDto.author);

    const newMessage = new Chat();
    newMessage.author = user;
    newMessage.room = room;
    newMessage.body = messageDto.body;
    newMessage.files = messageDto.files;
